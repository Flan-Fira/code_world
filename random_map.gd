...
func _ready():
	...
	for i in range(0,width):
		for j in range(0,height):
			if grid[i][j] == -2: get_node("TileMap 2").set_cell(i,j,1) #random tile: 
			if i == 0 or j == 0 or i == width-1 or j == height-1:
				grid[i][j] = -1
				get_node("TileMap 2").set_cell(i,j,0) #set border
	for i in range(0,width):  #set door
		for j in range(0,height):
			for x in [((width/2)-1), (width/2), ((width/2)+1)]: #3 middle spots of row for top and bottom rows
				for y in [0, height-1]: 
					get_node("TileMap 2").set_cellv(Vector2 (x,y),2) #set black tile
			for x in [0, height-1]: #3 middle spots of collumn for first and last columns
				for y in [((width/2)-1), (width/2), ((width/2)+1)]:
					get_node("TileMap 2").set_cellv(Vector2 (x,y),2) #set black tile
...
func grass_rock(curr,dirv,len):
	for i in range(1,len,1):
		...
		get_node("TileMap 2").set_cell(curr.x,curr.y,1) # set inside
		#rock = rock + 1
